...
describe('Developer Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./developer-detail.component').then(m => m.DeveloperDetailComponent),
              resolve: { developer: () => of({ id: 22287 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load developer on init', async () => {
      ...
      // THEN
      expect(instance.developer()).toEqual(expect.objectContaining({ id: 22287 }));
    });
  });
