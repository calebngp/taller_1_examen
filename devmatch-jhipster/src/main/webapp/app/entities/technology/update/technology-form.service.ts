...
type TechnologyFormDefaults = Pick<NewTechnology, 'id' | 'developers' | 'projects'>;
...
type TechnologyFormGroupContent = {
  ...
  developers: FormControl<ITechnology['developers']>;
  projects: FormControl<ITechnology['projects']>;
};
...
export class TechnologyFormService {
  createTechnologyFormGroup(technology: TechnologyFormGroupInput = { id: null }): TechnologyFormGroup {
    ...
    return new FormGroup<TechnologyFormGroupContent>({
      ...
      developers: new FormControl(technologyRawValue.developers ?? []),
      projects: new FormControl(technologyRawValue.projects ?? []),
    });
  }
